...
app.get('/viewquestionsall/:id', function(req, res) {
  ...
  client.connect();
  client.query("(SELECT DISTINCT questions.id,title, description, users1.first_name, users1.last_name,questions.status,questions.date,answers.id_owner FROM questions INNER JOIN users as users1 ON questions.id_owner=users1.id INNER JOIN question_topic ON question_topic.id_question=questions.id and question_topic.topic=(SELECT level FROM users as users2 WHERE users2.id=$1) LEFT JOIN answers ON answers.id_question=questions.id  and answers.id_owner =$1 WHERE questions.status='open' and answers.id_owner is NULL ORDER BY date DESC)UNION(SELECT DISTINCT questions.id,title, description, users1.first_name, users1.last_name,questions.status,questions.date,answers.id_owner FROM questions INNER JOIN users as users1 ON questions.id_owner=users1.id INNER JOIN question_topic ON question_topic.id_question=questions.id and question_topic.topic=(SELECT id_index_category FROM users as users2 WHERE users2.id=$1) LEFT JOIN answers ON answers.id_question=questions.id  and answers.id_owner =$1 WHERE questions.status='open' and answers.id_owner is NULL ORDER BY date DESC) UNION (SELECT DISTINCT questions.id,title, description, users1.first_name, users1.last_name,questions.status,questions.date,answers.id_owner FROM questions INNER JOIN users as users1 ON questions.id_owner=users1.id INNER JOIN question_topic ON question_topic.id_question=questions.id and question_topic.topic IN (SELECT DISTINCT weather FROM user_weather WHERE user_weather.id_user=$1) LEFT JOIN answers ON answers.id_question=questions.id  and answers.id_owner =$1 WHERE questions.status='open' and answers.id_owner is NULL ORDER BY date DESC)"
,[req.params.id])
  .then(res1 => {
    client.end();
    res.send(res1.rows);
